...
- Operate-mode density: two-row header (brand + theme, then step rail), full-bleed upload/review panels, dense match table
...
- **Pure Surface** (`surface`): Header bar, table scrollport, modals, inputs, inactive filter chips.
...
### Hierarchy
- **Title** (700, ~1.25–1.45rem, tight): Brand product name in the header (`1.25rem`); section heads in panels (`1.35rem`); completion head slightly larger (`1.45rem`). No separate marketing display size.
...
- **Label** (600, `0.875rem`): Field labels, step text, theme control, filter chips. Button / drop titles use control size (`0.9rem` / 600). Table body cells use compact size (`~0.82rem`; denser still in the desktop shell).
...
Operate-mode app shell: full-width header on the paper field, content in a centered `1200px` container with horizontal padding `24px` (`--space-lg`) and bottom padding `48px` (`--space-2xl`), each side expanded with `env(safe-area-inset-*)` when present. Header is always two rows: brand + right-justified theme control on row one; full-width step rail under the title on row two. Below `900px`, the review toolbar stacks and search/actions go full-width. Viewport meta uses `viewport-fit=cover` for notched devices.

Upload and review work surfaces are full-bleed panels on the paper/desk field—no nested Surface Lift “job cards.” Upload uses a two-column drop grid (`16px` gap) that collapses to one column at `640px`; supporting copy and security notes stay within a `65ch` measure. Review is denser: a hairline-separated toolbar (stats | find cluster + actions), then a bordered table scrollport with compact cell padding and fixed column widths tuned for a ~750px desktop window—keep the table horizontally scrollable on overflow rather than cardifying rows. Fine-pointer browsers and the pywebview desktop shell (defaults to **750×660**, `min_size` 700×600) use flex-filled main content (no page scrollbar; the table scrolls). Touch/coarse pointers keep a roomier scrollable document flow. Vertical rhythm uses the documented space scale (`4 / 8 / 16 / 24 / 28 / 48`). Footer is a quiet centered meta row under a hairline.
...
Hybrid of hairline borders and one soft ambient lift. Modals and toasts share a single dual-stop shadow; primary upload/review panels stay flat on the paper field. The header uses a subtler 1px under-edge. Depth is structural (overlays and chrome), not nested card theater.

### Shadow Vocabulary
- **Surface Lift** (`box-shadow: 0 1px 3px rgba(15, 23, 42, 0.08), 0 8px 24px rgba(15, 23, 42, 0.06)`): Modals, toasts.
...
### Panels / Containers
- **Work panels:** Flat on Cool Paper / Dark Desk—no nested Surface Lift card around upload, review toolbar, or completion.
- **Table scrollport:** Pure Surface fill, `1px` Hairline Border, `8px` radius; fills leftover height in the desktop shell.
- **Toolbar:** Flush band with bottom hairline; compact padding (tighter still under `data-desktop`).
- **Overlays:** Modals keep Surface Lift + `12px` radius (see Modal).
...
### Navigation
- **Chrome:** Surface header with bottom hairline + hair shadow; row one = brand left + theme control right; row two = step rail full width under the title
- **Steps:** Muted label + circled number; active = Trust Blue fill/ring; done = Match Green Fill (`success-fill`) with `on-success` ink. Labels are **Upload → Review → Download** on web and **Read → Review → Save** in the desktop shell (`AK_DESKTOP`).
- **Separators:** `24×2px` border-colored bars between steps (`16px` wide in the desktop shell)
...
### Review Table
- **Header:** Sticky slate/surface band within a scrollport (`max-height: min(70vh, 720px)` on web; flex-fill / no max-height in the desktop shell); uppercase micro labels; caption is visually hidden for assistive tech
- **Columns:** Fixed layout with narrow Status / Actions columns; Aegis and KeePass share the flexible middle; Match Info stays compact
- **Rows:** Alternating near-white stripe; matched rows success-soft; modified rows amber inset bar (`3px`); cell text ~`0.82rem` (denser in desktop) with `overflow-wrap: anywhere`
...
### Theme Toggle
- Right-justified on the brand row (above the step rail); cycles System → Light → Dark
...
- **Do** keep upload/review work panels flat on Cool Paper; reserve Surface Lift for modals and toasts.
- **Do** preserve the AK mark + product name as the header identity pair; keep the subtitle as one muted utility line; keep steps under that title block.
...
- **Don't** use pill radii on primary buttons or wrap upload/review jobs in nested floating cards; overlays may elevate, task panels should not.
